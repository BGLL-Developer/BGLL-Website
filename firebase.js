...
import { getFirestore, getDocs, collection, query, where } from "https://www.gstatic.com/firebasejs/10.11.0/firebase-firestore.js";
...
function queryWinningNumbers(db, collectionName, date, status, callback) {
    ...
    const q = query(ref, where("date", "==", date), where('status', '==', status));
...
function listenForChangesBoledo() {
    ...
    daysOfWeek.forEach((day, index) => {
        ...
        queryWinningNumbers(db, "Boledo", formattedDate, 'active', (winningNumbers) => {
...
function listenForChangesJackpot() {
    ...
    queryWinningNumbers(db, "Jackpot", formattedDate, 'active', (winningNumbers) => {
...
function todayWinningNumber(dateString) {
    ...
    queryWinningNumbers(db, "Boledo", formattedDate, 'active', (winningNumbers) => {
        if (winningNumbers.length > 0) {
            ...
            document.getElementById('date').innerText = isNaN(parsedDate.getDate()) ? dateString : parsedDate.toDateString();
